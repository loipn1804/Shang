...
public class NotificationApi {
    ...
    private String URL = Constant.BASE_URL + "api/rest/extend/notifications";
    private HttpTransport HTTP_TRANSPORT = new NetHttpTransport();
    private int TIME_OUT = 60 * 1000;

    public void getAll(ApiCallback callback, long customer_id, int page) {
        try {
            GenericUrl requestUrl = new GenericUrl(URL + "?customer_id=" + customer_id + "&page=" + page);
...
//            HttpRequest request = requestFactory.buildPostRequest(requestUrl, ByteArrayContent.fromString("application/json", body));
    ...
    public void deleteById(ApiCallback callback, long id) {
        try {
...
//            HttpRequest request = requestFactory.buildPostRequest(requestUrl, ByteArrayContent.fromString("application/json", body));
    ...
    public void updateById(ApiCallback callback, long id, String body) {
        try {
            GenericUrl requestUrl = new GenericUrl(URL + "/" + id);
