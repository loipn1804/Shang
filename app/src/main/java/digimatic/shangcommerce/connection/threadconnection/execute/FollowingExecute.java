...
import digimatic.shangcommerce.connection.api.FollowingApi;
 ...
 */
public class FollowingExecute {

    public static void getListFollowing(final Context context, final UICallback callback, final long customer_id) {
        ...
        BackgroundThreadExecutor.getInstance().runOnBackground(new Runnable() {
            ...
            public void run() {
                ...
                FollowingApi api = new FollowingApi();
                api.getListFollowing(apiCallback, customer_id);
            }
        });
    }

    public static void follow(final Context context, final UICallback callback, final String body) {
        if (!StaticFunction.isNetworkAvailable(context)) {
            callback.onResult(false, context.getString(R.string.no_internet));
            return;
        }
        BackgroundThreadExecutor.getInstance().runOnBackground(new Runnable() {
            ...
            public void run() {
                ...
                FollowingApi api = new FollowingApi();
                api.follow(apiCallback, body);
            }
        });
    }

    public static void unfollow(final Context context, final UICallback callback, final long id, final String body) {
        if (!StaticFunction.isNetworkAvailable(context)) {
            callback.onResult(false, context.getString(R.string.no_internet));
            return;
        }
        BackgroundThreadExecutor.getInstance().runOnBackground(new Runnable() {
            ...
            public void run() {
                ...
                FollowingApi api = new FollowingApi();
                api.unfollow(apiCallback, id, body);
            }
        });
    }
}
